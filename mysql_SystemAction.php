...
switch($act) {
	...
	case 'menusystemlist' :
		...
		case 'menusubsystemlist' :
			$retval = menusubsystemlist();
			break;
...
where active=1 and  E_code='".$_GET['p_e_code']."'";
	   ...
	   switch($loc) 
		{
      	case 'cpxsdsh' :
		   $khkd=$_GET["khkd"];
		   $filter .=" and cpxsd.ztbz=0 and cpxsd.delbz=0 and khkd=".$khkd;
		   break;
	  	case 'cpxsdmfh' :
...
function cpgfdmxsave() {
	...
	$arr=mysql_fetch_assoc($result);
	$dh =$arr['dh'];
	$cpgfdstr = " insert into cpgfd (gfdh,L_id,khid,khmc,cphm,sfr,czy,cnote,zl,je,xjje,xjbz,gfrq)values('";
	$cpgfdstr .= $dh. "'";
	$cpgfdstr .= "," . $L_id;
	$cpgfdstr .= "," . $o['khid'];
	$cpgfdstr .= ",'" . $o['khmc'] . "'";
     ...
     foreach ($cpckdmx as $row) {
			...
			foreach ($cpckdje as $jerow) {
				if ($xsmxid==$jerow['mxid']){
					$cpckdjestr = " insert into cpckdje (work,dw,sl,dj,je,workid,ckmxid,xjbz,zljs,inbz,indj)";
	...
	foreach ($params as $arr) {
	 ...
	 case 1 :
		if ($table=="khusers"){
			...
			$e_code=$_GET['p_e_code'];
			$sql = "insert into users(typeid,e_code,lidstring,usercode,username,tel,sfjhm,qqnumber,
...
if ($khid>0){
	$lid=$_GET['p_l_id'];
	$sql = "insert into packing_kh (PS_id,khid,L_id,mints,Czdj,Phdj,Czdj2,Phdj2,Pfdj,Bydj,Pbdj,Ghdj) values(" . $arr['Pid'] ;
	$sql .= "," . $khid;
	$sql .= "," . $lid;
	$sql .= "," .$arr['mints'];
}
...
{
	$sql = "insert into packing(PS_code,E_code,PS_name,Quantity_Unit,Weight_Unit,Rate,Weight_Status,Czdj,Phdj,Czdj2,Phdj2,Pfdj,Bydj,Pbdj,Ghdj) values('" . $arr['PS_code'] . "'";
	$sql .= ",'" . $arr['E_code'] . "'";
	$sql .= ",'" . $arr['PS_name'] . "'";
	$sql .= ",'" . $arr['Quantity_Unit'] . "'";
	$sql .= ",'" . $arr['Weight_Unit'] . "'";
...
function menusubsystemlist() {

	$termtype = $_GET['termtype'];
	$appid = $_GET['appid'];
	$subsystemid = $_GET['subsystemid'];
    //return $khsysyem;
	$s = base64_decode($_GET['userInfo']);
	$o = json_decode($s);
	$o = json_decode($o, true);
   ...
   if ($khsystem=="1")
   {
	$sqlstr = "SELECT systemname,menu,sort,NAME,iconurl,viewpath ,menu_id,widgetName,enabled,khbz,alluser FROM v_systemmenu 
	WHERE  systemid=".$subsystemid ." and menu_id<>19 AND menu_id<>22 AND (alluser=1 OR ( enabled=1 AND khbz>0 ) ) ORDER BY barsort,Sort ";
   }
   else
   {
	$menustring="";
	...
	$sqlstr = " SELECT systemname,menu,sort,NAME,iconurl,viewpath ,menu_id,widgetName,enabled,khbz,alluser FROM v_systemmenu WHERE  enabled=1 and systemid=".$subsystemid;
		
		
		if ($appid == "2"){
			$sqlstr .=" and ( khbz=1 or alluser=1)"; 
		}else
		{
			if ($menustring>"")
			{
				$sqlstr .= " and ( alluser=1 or ( khbz<3  and (name='-' or  POSITION( CONCAT(';',menu_id ,';')  in '".$menustring."')>0 )))";		
			}else
			{
				$sqlstr .= " and (khbz<3  or alluser=1 )";		

			}
		}
		$sqlstr .= "  order by barsort,Sort ";
