...
Ext.define('MyApp.view.main.cpckgl.CpckdckshCtrl',
{
    ...
    requires: [
        ...
        'MyApp.view.main.tree.WorkerSelectTree', 'MyApp.view.main.cpckgl.CpckdmxShEdit'
    ],
    locQuery: function (that)
        ...
        CpckdshSave: function (loc, the)
        {
            ...
            cpckdmx_store.each(function (rec)
            {
                ...
                else
                {
                    ...
                    if ((Math.round(rec.data.ccsl * 1000) != Math.round(sumsl * 1000)) || (Math.round(rec.data.cczl * 1000) != Math.round(sumzl * 1000)))
                    {
                        ckstatus = 1;
                    }
                }
            ...
            if (loc == 'ok')
            {
                if (ckstatus == 1)
                {
                    Ext.MessageBox.alert("注意！", "输入出仓数量、重量与仓位明细出仓内容不一致！</br>请补录入仓位出仓明细数据！");
                    return;
                }
            }
            var cksh = {};
            ...
            Ext.MessageBox.show(
            {
                ...
                fn: function (btn, text)
                {
                    if (btn == "yes")
                    {
                        ...
                        AjaxDataSave('cpckdckshsave', loc, encodedString, ckckshsaveCallBack, the);
                    }
                    else{
                        the.lookupReference('popupCpckdWindow').down("#btnCpckdDelete").setHidden(false);
                        the.lookupReference('popupCpckdWindow').down("#btnCpckdSave").setHidden(false);
                    }
                }
            });
        },
        onCpckdmxFormSubmit: function ()
        {
            ...
            if ((sumsl != 0) || (sumzl != 0))
            {
                if ((Math.round(sumsl * 1000) != 1000 * p.get('ccsl')) || (Math.round(sumzl * 1000) != 1000 * p.get('cczl')))
                {
