...
Ext.define('MyApp.view.main.cpckgl.CpckdCtrl',
{
    ...
    requires: [
        'MyApp.view.main.cpckgl.CpckdView', 'MyApp.view.main.cpckgl.CpckdCtrlFunction', 'MyApp.view.main.report.PrintCpckd'
    ],
    locQuery: function (the)
        ...
        onCpckdjeAddClick: function (record)
        {
            ...
            treeSelect('work', that, obj, that.popupmx, false, ckdworkCallBack,'请选择装卸项目：');
            return false;
        },
        ...
        onCpckdFormSubmit: function ()
        {
            ...
            cpckdje_store.each(function (recje)
            {
                if (recje.get('sl') != 0)
                {
                    if ((sumjesl < 1) && (s == 0) && (recje.get('zljs')))
                    { //重不够吨按一吨计
