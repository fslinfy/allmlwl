Ext.define('MyApp.view.main.report.PrintCpxsd',
{
    extend: 'Ext.Mixin'
});
function printCpxsd(p)
{
    var mx = p.mxrec;
    if (!LODOP)
    {
    ...
    str = str + '<thead>';
    str = str + '<tr><th width="100%" colspan="8"><table width="100%" border=0 ><tr><td style="border:0;" ></td><td style="text-align:right;border:0;width:50;">No:</td><td style="text-align:left;border:0;width:130;" ><strong>' + p.xsdh + '</strong></td></tr></table></th></tr>';
    ...
    LODOP.ADD_PRINT_BARCODE(5, 5, 120, 120, "QRCode", p.xsdh);
    if (p.delbz)
    {
...
function onPrintCpxsd()
{
    PrintCpxsdxsid(xsid);
};
function PrintCpxsdxsid(xsid)
{
    if (xsid == 0)
    {
        return;
    ...
    var prtxsmxStore = Ext.create('Ext.data.Store',
    {
    ...
    var prtxsStore = Ext.create('Ext.data.Store',
    {
    ...
    prtxsmxStore.on("load", function ()
    {
        prtxsStore.load();
    });
    prtxsStore.on("load", function ()
    {
        prtxsd(prtxsStore, prtxsmxStore);
    });
    prtxsmxStore.load();
}
function prtxsd(prtxsStore, prtxsmxStore)
{
    var mxrec = [];
    var gsbyrec = {};
    var i = 0;
    prtxsStore.each(function (p)
    {
       var sumsl = 0,
            sumzl = 0,
            sumje = 0;
        prtxsmxStore.each(function (rec)
            {
        ...
        mxrec.push(gsbyrec);
       var xsd = p.data;
        xsd["xsrq"] = Ext.Date.format(p.data.xsrq, 'Y-m-d');
        xsd["endrq"] = Ext.Date.format(p.data.endrq, 'Y-m-d');
        xsd["mxrec"] = mxrec;
        xsd["xsje"] = 0;
        xsd["xssl"] = sumsl.toFixed(3);
        xsd["xszl"] = sumzl.toFixed(3);
        printCpxsd(xsd);
    })
}