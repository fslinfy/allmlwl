...
function printcptzd(p)
{
    ...
    str = str + '<caption><br><b><font face="黑体" size="5">商品调账单</font></b></caption>'
    str = str + '<thead>'
    str = str + '<tr><th width="100%" colspan="14"></th></tr>'
    str = str + '<tr><th width="100%" colspan="14"><table width="100%" border=0 ><tr>';
    str = str + '<td style="text-align:left;border:0;width:70;" >客户名称:</td><td style="text-align:left;border:0;"><strong>' + p.khmc + '</strong></td>';
    str = str + '<td style="text-align:left;border:0;width:70;" >调账仓库:</td><td style="text-align:left;border:0;"><strong>' + p.ckmc + '</strong></td>';
    str = str + '<td style="text-align:left;border:0;width:30;" >No:</td><td style="text-align:left;border:0;width:120;"><strong>' + p.tzdh + '</strong></td></tr></table></th></tr>';
    str = str + '<tr><th width="100%" colspan="14"><table width="100%" border=0 ><tr>';
    str = str + '<td style="text-align:left;border:0;width:70;" >调入客户:</td><td style="text-align:left;border:0;"><strong>' + p.newkhmc + '</strong></td>';
    str = str + '<td style="text-align:left;border:0;width:70;" >调账费用:</td><td style="text-align:left;border:0;"><strong>' + p.tzje + '</strong></td>';
    if (p.jekh)
    {
        str = str + '<td style="text-align:left;border:0;width:60;">支付方:</td><td style="text-align:left;border:0;"><strong>调入方</strong></td>';
    }
    else
    {
        str = str + '<td style="text-align:left;border:0;width:60;">支付方:</td><td style="text-align:left;border:0;"><strong>调出方</strong></td>';
    }
    str = str + '<td style="text-align:right;border:0;width:70;">调账日期:</td><td style="text-align:left;border:0;width:120;" ><strong>' + p.tzrq + p.rq.substr(10, 6) + '</strong></td></tr></table></th></tr>';
    //表头
    str = str + '<tr>';
    ...
    str = str + '<td  style="text-align:center;width:35;"><strong>单位</strong></td>';
    ...
    str = str + '<td  style="text-align: center;width:80;"><strong>进库日期</strong></td>';
    ...
    mx.forEach(function (rec)
    {
        if (rec.mxdh == '1')
        {
            ...
            str = str + '<td rowspan="2" style="text-align:center;">' + trim(rec.jldw) + '</td>';
            str = str + '<td rowspan="2" style="text-align:right;">' + printsl(rec.tzsl) + ' </td>';
            str = str + '<td rowspan="2" style="text-align:right;">' + printsl(rec.tzzl) + ' </td>';
            str = str + '<td style="text-align:center;">' + trim(rec.cw) + '</td>';
            str = str + '<td>' + trim(rec.cpph) + '</td>';
            str = str + '<td  style="text-align:center;">' + printsl(rec.czdj) + '</td>';
            str = str + '<td  style="text-align:center;">' + Ext.Date.format(rec.czrq, 'Y-m-d') + '</td>';
            ...
            str = str + '<td  style="text-align:center;">' + printsl(rec.newczdj) + '</td>';
            str = str + '<td  style="text-align:center;">' + Ext.Date.format(rec.newczrq, 'Y-m-d') + '</td>';
        ...
        else
        {
            ...
            str = str + '<td  style="text-align:right;">' + slrenderer(rec.tzsl) + '</td>';
            str = str + '<td  style="text-align:right;">' + slrenderer(rec.tzzl) + '</td>';
            str = str + '<td>' + trim(rec.cw) + '</td>';
            str = str + '<td>' + trim(rec.cpph) + '</td>';
            str = str + '<td  style="text-align:center;">' + slrenderer(rec.czdj) + '</td>';
            str = str + '<td  style="text-align:right;">' + slrenderer(rec.tzje) + '</td>';
            str = str + '<td  style="text-align:right;"></td></tr>';
        }
    })
    str = str + '<tr><td style="text-align:center;">汇总 </td><td  style="text-align:left;"  colspan="3"><strong>' + lowMoneyToUp(p.tzje) + '</strong></td>';
    str = str + '<td></td><td style="text-align:right;"><strong>' + slrenderer(p.tzsl, 0) + '</strong></td><td style="text-align:right;"><strong>' + slrenderer(p.tzzl, 0) + '</strong></td>';
    str = str + '<td></td><td></td><td style="text-align:center;"></td><td  style="text-align:right;"><strong>' + slrenderer(p.tzje) + '</strong></td><td></td></tr>';
    ...
    str = str + '</tbody>'
    str = str + '<tfoot>'
    str = str + '<tr><th width="100%" colspan="12"><table width="100%" border=0 ><tr><td  style="text-align:left;border:0;width:20%;" >制单:' + p.czy + '</td>';
    str = str + '<td  style="text-align:left;border:0;width:20%;">审核:' + p.shr + '</td>';
    str = str + '<td  style="text-align:left;border:0;width:20%;">仓管员:</td>';
    str = str + '<td  style="text-align:left;border:0;width:20%;">叉车员:</td>';
    str = str + '<td  style="text-align:left;border:0;width:20%;">搬运员:</td></tr></table></th></tr>'
    str = str + '<tr><th ></th></tr>'
    str = str + '</tfoot></table>';
    ...
    if (p.delbz)
    {
        LODOP.ADD_PRINT_TABLE(80, 900, 130, 30, '<table width="100%" border=0 ><tr><td style="color:red;text-align:center;border:0; " ><strong>此单已作废!</strong></td></tr></table>');
    }
    LODOP.PREVIEW();
};
