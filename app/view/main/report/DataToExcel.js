...
function DataToExcel(op)
{
    ...
    var excel = '<table>';
    //设置标题
    excel += " <tr><td colspan=" + showLabel.length.toString() + " style='font-weight: 500; font-size:large; text-align: center;'>" + title + "</td></tr>"
    if ((smallTitle.length > 0) && (smallTitle1.length > 0))
    {
        var i1 = parseInt((showLabel.length) / 2);
        var i2 = showLabel.length - i1;
        excel += " <tr><td colspan=" + i1.toString() + " >" + smallTitle + "</td><td colspan=" + i2.toString() + " >" + smallTitle1 + "</td></tr>"
    }
    else
    {
        excel += " <tr><td colspan=" + showLabel.length.toString() + " >" + smallTitle + "</td></tr>"
    }
    //设置表头
    var row = "<tr>";
    for (var i = 0, l = showLabel.length; i < l; i++)
    {
        row += "<td style='color: #FFFFFF; font-weight: 700; font-size: large;background-color: #006699; '>" + showLabel[i] + '</td>';
    }
    //换行
    excel += row + "</tr>";
    //设置数据
