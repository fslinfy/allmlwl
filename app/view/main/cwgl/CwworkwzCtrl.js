sys_DisplayAll = "";
var that;
Ext.define('MyApp.view.main.cwgl.CwworkwzCtrl',
{
    extend: 'Ext.app.ViewController',
    alias: 'controller.CwworkwzCtrl',
    requires: [
        'MyApp.view.main.cwgl.CwworkwzView'
    ],
    locQuery: function (the)
        {
            ...
            var ny = v.get('ny');
            var yu = v.get('yu');
            CwworkwzStore.proxy.extraParams.p_l_id = ckid;
            CwworkwzStore.proxy.extraParams.khid = khid;
            CwworkwzStore.proxy.extraParams.bz = "";
            CwworkwzStore.proxy.extraParams.ny = ny;
            CwworkwzStore.proxy.extraParams.yu = yu;
            CwworkwzStore.reload();
        },
        ...
        getexcelsheetdata: function (dataarray, khid, khmc, ny, yu)
        {
            ...
            for (var i = 0; i < dataarray.length; i++)
            {
                ...
                if (oldobj.khid == khid)
                {
                    ...
                    arr.push(
                    {
                        'text': slrenderer(oldobj.kcsl0)
                    });
                    arr.push(
                    {
                        'text': slrenderer(oldobj.kczl0)
                    });
                    arr.push(
                    {
                        'text': slrenderer(oldobj.jcsl)
                    });
                    arr.push(
                    {
                        'text': slrenderer(oldobj.jczl)
                    });
                    arr.push(
                    {
                        'text': slrenderer(oldobj.ccsl)
                    });
                    arr.push(
                    {
                        'text': slrenderer(oldobj.cczl)
                    });
                    arr.push(
                    {
                        'text': slrenderer(oldobj.tzsl)
                    });
                    arr.push(
                    {
                        'text': slrenderer(oldobj.tzzl)
                    });
                    arr.push(
                    {
            ...
            if (sumrows > 0)
            {
                ...
                arr.push(
                {
                    'text': slrenderer(sumkcsl0)
                });
                arr.push(
                {
                    'text': slrenderer(sumkczl0)
                });
                arr.push(
                {
                    'text': slrenderer(sumjcsl)
                });
                arr.push(
                {
                    'text': slrenderer(sumjczl)
                });
                arr.push(
                {
                    'text': slrenderer(sumccsl)
                });
                arr.push(
                {
                    'text': slrenderer(sumcczl)
                });
                arr.push(
                {
                    'text': slrenderer(sumtzsl)
                });
                arr.push(
                {
                    'text': slrenderer(sumtzzl)
                });
                arr.push(
                {
                    'text': slrenderer(sumkcsl)
                });
                arr.push(
                {
                    'text': slrenderer(sumkczl)
                });
                jsonData.push(arr);
            }
            return jsonData;
