function treeSelect(mc, the, kcbz, viewname, resh, callback,cTitle)
{
    if (cTitle==null || cTitle=="") cTitle='请选择：';
    ...
    {
    case "ckmc":
        url = sys_SystemActionPHP + '?act=locationselecttreelist';
        this.f_id = 'ckid';
        this.f_mc = 'ckmc';
        cTitle="请选择仓库：";
        height = 300;
...
function system_setting()
{
    ...
    var appid = Cookiecp.get('apptypeid');
    if (appid == undefined)
    {
        if (sys_appid == 1) return;
    }
    var appid = 1;
...
function user_login()
{
    ...
    var Cookiecp = Ext.create('Ext.state.CookieProvider');
    var appid = Cookiecp.get('apptypeid');
    sys_customer_id = Cookiecp.get('sys_customer_id');
    if (sys_customer_id == undefined) sys_customer_id = 0;
    if (appid == undefined)
    {
        if (sys_appid != 1)
        {
    ...
    apploginWin = new Ext.Window(
    {
        width: 360,
        height: 400,
        title: (sys_system_name==undefined)?'用户登录':sys_system_name+'-用户登录',
        plain: true,
        resizable: false,
        frame: true,
        modal: true,
                ...
                success: function (form, action)
                    {
                        ...
                        if (parseInt(result.userid) > 0)
                        {
                            ...
                            {
                                if ((Tongji(lidstring, ',') != 2) && (sys_appid == 1))
                                {
...
function pagereset()
{
    ...
    }, "->",
    {
        ...
        handler: function ()
        {
            var Cookiecp = Ext.create('Ext.state.CookieProvider');
            var appid = Cookiecp.get('apptypeid');
            if ((appid == 0 || appid == undefined) && (sys_appid != 1))
            {
                system_setting();
            }
            else
...
function getsystem(str)
{
    Ext.Ajax.request(
    {
        ...
        success: function (req)
        {
            ...
            var Cookiecp = Ext.create('Ext.state.CookieProvider');
            var appid = Cookiecp.get('apptypeid');
            if (sys_appid == 0 && (appid == 0 || appid == undefined))
            {
                system_setting();
            }
            else
...
function createmenu()
{
    ...
    if (sys_subsystemid > 0)
    {
        var URL = sys_SystemActionPHP + '?act=menusubsystemlist&termtype=classic&appid=' + appid + "&subsystemid=" + sys_subsystemid + " &userInfo=" + base64encode(Ext.encode(obj2str(sys_userInfo))); //+"&khsystem="+sys_userInfo.khsystem;
    }
    else
    {
        var URL = sys_SystemActionPHP + '?act=menusystemlist&termtype=classic&appid=' + appid + "&userInfo=" + base64encode(Ext.encode(obj2str(sys_userInfo))); //+"&khsystem="+sys_userInfo.khsystem;
    }
    Ext.Ajax.request(
    {
        url: URL,
        ...
        success: function (req)
            {
                ...
                menu0.forEach(function (value, index, array)
                {
                    if (cbarmenu != value.menu)
                    {
                        ...
                        if (sys_appid > 0)
                        {
                            document.title = value.systemname;
                        }
                        submenu = [];
                        m = 0;
                        oldmenu = "";
